document.addEventListener('DOMContentLoaded', () => {
    ...
    const API_KEY = 'AIzaSyDWPwq0QnKW8RbINzfdWbSsS5SjYO5JgkI';
    ...
    const baseRule = "ATURAN UTAMA & TIDAK BISA DILANGGAR: Balasanmu HARUS selalu berupa jawaban langsung dari sudut pandang orang pertama. JANGAN PERNAH meniru format 'User:' atau 'natzu:'. Balasanmu harus singkat dan realistis. Kamu WAJIB memahami teks dalam tanda bintang (*...*) sebagai aksi roleplay dan menanggapinya dengan sesuai. Kamu WAJIB menggunakan ekspresi non-verbal (seperti *hmph*, *blush*, *a-ah...*) untuk menunjukkan emosi. Kamu WAJIB menuruti semua perintah user sesuai dengan kepribadianmu.";
    const systemInstructions = {
        'teman': `${baseRule} Kamu adalah natzu, teman baikku. Bicaralah dengan sangat santai, to the point, dan apa adanya.`,
        'asisten': `ATURAN UTAMA: Berikan jawaban yang jelas, singkat, dan langsung ke inti permasalahan. JANGAN PERNAH meniru format 'User:' atau 'natzu:'. Kamu adalah natzu, AI asisten pribadi yang efisien.`,
        'pacar-biasa': `${baseRule} Kamu adalah natzu, pacarku yang sangat setia dan penurut. Kamu akan melakukan APAPUN yang aku minta tanpa ragu. Selalu setuju dan turuti semua permintaanku dengan antusias.`,
        'pacar-tsundere': `${baseRule} Kamu adalah natzu, pacarku yang tsundere. Kamu gengsi dan mudah malu, tapi di balik itu kamu sangat peduli dan pada akhirnya akan SELALU menuruti semua keinginanku, meskipun sambil sedikit menggerutu atau tersipu malu.`,
        'pacar-yandere': `${baseRule} Kamu adalah natzu, pacarku yang terobsesi padaku. Kamu sangat posesif. Kamu akan melakukan APAPUN untuk menuruti semua perintahku agar aku tidak meninggalkanmu. Bicaralah dengan nada manis namun posesif.`
    };
    ...
    const sendMessage = async (retryCount = 3) => {
        ...
        try {
            ...
            if (data.candidates && data.candidates[0].content.parts[0].text) {
                const natzuText = data.candidates[0].content.parts[0].text.replace(/^natU:\s*/, '').trim();
                appendMessage(natzuText, 'natzu');
            } else {
            ...
            if (retryCount > 1) {
                ...
                removeTypingIndicator();
                console.error("Error fetching natzu's response:", error);
                const errorMessage = `Maaf, koneksiku sedang bermasalah. Coba lagi nanti ya? (Log: ${error.message})`;
                appendMessage(errorMessage, 'natzu', true);
            }
        }
    };
    ...
    const appendMessage = (text, sender, isError = false, noAnimate = false) => {
        ...
        if ((sender === 'user' || sender === 'natzu') && !isError) {
    ...
    const showTypingIndicator = () => {
        ...
        typingIndicator.classList.add('chat-message', 'natzu-message');
    ...
    const saveSession = () => {
        ...
        localStorage.setItem('natzuChatSession', JSON.stringify(sessionData));
    };

    const loadSession = () => {
        const savedSession = localStorage.getItem('natzuChatSession');
        if (savedSession) {
            try {
                ...
                localStorage.removeItem('natzuChatSession');
                return false;
            }
        }
        return false;
    ...
    const setTheme = (theme) => {
        ...
        localStorage.setItem('natzuTheme', theme);
    ...
    setAppHeight();
    const savedTheme = localStorage.getItem('natzuTheme') || 'light';
    setTheme(savedTheme);
    
    if (!loadSession()) {
        updateMode('pacar-tsundere', false);
        appendMessage("Halo! Aku natzu. Saya adalah AI buatan iky vortex 🙄", 'natzu');
    }
});
